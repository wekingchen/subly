...
2. 准备一个持久化目录映射到容器内 `/app/data`。容器入口会自动修复旧版 root-owned 数据卷权限，再以非 root UID/GID `10001` 运行 Uvicorn；使用宿主机目录时需允许容器 root 调整 ownership。请保持默认入口运行；若 NAS 强制指定运行用户，必须设为 `10001:10001`，其他 UID 会被启动拒绝。
3. 准备以下环境变量。

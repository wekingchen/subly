...
- 信用卡管理页新增「全部账单」跨卡对账视图（页头按钮与待还款统计卡点击进入）：跨卡列出全部已解析历史账单（含已删卡的孤立账单，标注「已删卡 / 未关联 · 银行 ···· 尾号」），支持单期标记/取消还款与部分还款——此前已删卡账单虽计入待还总额与逾期统计却无处可见、无法标记；标记后待还总额与逾期统计实时刷新（孤立账单同样生效）；列表附统计口径说明（历史账单不可直接相加）与 500 条展示上限提示。
- 信用卡管理页卡片新增「年费可豁免」徽标：免年费进度达标（刷免笔数/金额满足任一副目标）的卡在列表卡片上直接显示金色达标标，无需逐张点开详情确认；新增批量达标汇总接口（一次请求算出全部卡的达标状态），卡片编辑或账单变更后徽标实时刷新。
- 免年费进度支持「本期无消费自动跳期」：同一银行多张卡时，账单邮件里其他卡有本期账单而某张卡没有被列出（该卡本期无消费）——该期自动判定为「无消费已跳过」，不再误报「缺少账单数据」、不再触发无意义的补拉（证据要求历史邮件组关系——该他卡尾号曾与本卡共享同封合并邮件——且勾稽通过；无组关系的他卡账单、勾稽失败账单不算证据）；详情页进度块中性提示「N 期无消费已跳过」，已知限制：同银行全部卡同期零消费时仍会提示缺账单（纯派生无证据可依）。
- 信用卡账单支持部分还款（多次还清）：卡片与账单明细的还款操作改为「登记还款」弹窗——输入当次还款金额（默认填当前剩余待还全额，一次还清点确认即可；焦点清空输入便于直接键入），未还清时账单继续显示剩余待还（卡片待还金额与提醒文案同步扣减，下次再还），能可靠确定账单所属还款期时累计还清后自动顺延到下个账单周期、静默当期提醒，并把金额已滚入本期的更早未标记账单一并结清（避免只还最新一期后旧期欠款「复活」）；还款日缺失或历史账单期无法可靠判定时保守不推进（宁多提醒一期，不错误静默其他未还账单）。还款按剩余待还上限响亮校验（超出部分拒绝，不产生负待还）；账单明细区「取消还款标记」会同时清除已登记的还款记录（清零重算，错录金额的唯一修正入口，有确认提示）；备份导出/导入包含已还累计金额并校验其不超过应还。
- 订阅账本排序升级：每个分类内默认按剩余订阅时间由近及远排列（到期日越早越靠前，一次性买断/无日期订阅沉底）；用户手动拖拽过的分类顺序持久化保存（新增/删除/迁移成员后不再被默认排序覆盖）。已拖拽顺序与默认排序互不干扰。
- 雷达工作台的「已逾期」统计卡点击后不再跳转支出报表，改为打开逾期订阅列表弹窗：逐条展示名称、逾期天数与单次续费金额（按逾期时间升序），点击条目直接打开订阅详情标记续费，续费后列表与雷达计数实时更新；其余三张雷达卡（今天至 3 天 / 4–7 天 / 8–30 天）仍跳转续费日历。

- Bark 推送图标：订阅提醒无图标时回退 Subly 品牌 logo（`/pwa-192.png`），信用卡还款提醒携带所属银行官方徽标（与日历/卡片徽标同源，未收录银行回退品牌 logo），Bark 测试推送也携带品牌 logo——锁屏即可区分推送来源；需要配置 `APP_PUBLIC_URL`（地址必须能被接收推送的设备访问），未配置时显示 Bark 默认图标、推送不受影响。银行识别复用账单关联的 match_bank 模块（英文别名补齐并大小写不敏感，与前端徽标识别同口径）。

- 信用卡还款提醒文案升级（三通道同步）：标题含银行名与卡片名（「💳 招商银行 · 主卡 还款提醒」），正文含还款倒计时（「还有 N 天」/当天「今天」）与最新未还账单的应还金额（与待还汇总同口径：最新一期、勾稽通过、未标记还款，statement_date 缺失回退 bill_period_end）；最新账单为负按「账上有富余，本期无需还款」提示，金额未知时回退「金额以银行账单为准」不猜；Webhook event 新增结构化 `total_due`（可空，负数=富余）与 `days_before` 字段。交易明细、最低还款、额度与尾号仍不外发。

- 民生银行账单逐卡解析修正（真实 90 期账单验证 90/90 闭环）：多卡账单的账户级应还此前被原样复制给每张卡（两张卡显示相同金额），现按各卡有效正数消费占比分摊账户应还——分期放款等「贷记调整」不占权重、各卡之和恒等于账户总应还（勾稽闭环校验）；「自助转入」与「支付宝/富友支付-人名」等无「还款」字样的负数转账在民生路径保守归还款而非退款（此前误判退款会错误扣减免年费消费额，该启发式不进入其他银行路径）；上期未结清或账户合计解析失败时逐卡金额置空并跳过无法成立的闭环校验；上期已结清但全账户无有效正数交易（明细与记账不变量冲突）时逐卡金额置空并标记 mismatch（不猜、不伪装成 0）。存量数据更新容器后在设置页点「解析账单」自动按新口径刷新。


- 发布扫描链路的 Trivy 显式固定为 v0.74.0：此前扫描器版本由 `trivy-action` 的默认值间接决定，升级 Action 时可能连带改变；现 SARIF 报告与可修复 High/Critical 门禁统一显式声明版本。经核对，0.70→0.74 的上游变化不影响当前通过 image tar 执行的扫描参数与门禁流程（v0.72.0 的发布镜像 `dockers_v2` 迁移不涉及本仓库的 setup-trivy 二进制安装路径），后续版本升级人工规划。
- 信用卡待还款/富余口径修正为「按每卡最新账单」（滚动余额语义，用户确认）：银行账单应还总额本身已包含历史欠款，此前逐期累加会把同一笔钱重复计数——现在最新账单为正即该卡待还款、为负即「账上有富余」，富余不再以负数混入全局待还总额（新增 `surplus_total` 单独返回，不跨卡抵扣他卡欠款）；逾期同步随最新账单口径（旧期欠款已被滚动余额吸收，不再作为当前逾期本金累加），已删卡孤立账单组保持逐期累加的历史口径。免年费历史账单补拉成功后，严格早于该卡最新账单的未标记旧期自动标记已还款（用户无需逐个打标；不推进还款顺延界线，提醒节奏不变），只有最新一期需要用户手动标记。

- 修复 QQ 邮箱补拉历史账单超时报「请求失败」（生产实测根因）：QQ 邮箱 IMAP 对带外层括号的 SEARCH 日期条件（`(SINCE "…" BEFORE "…")`）静默忽略、返回全量邮箱（生产实测 INBOX 10551 封全部成为候选，头部拉取约 15 分钟必然超时），改为无外层括号的逐 token 形态（`SINCE … BEFORE …`）后同窗口仅 33 封；126 邮箱两种形态均正常，无括号为 IMAP 标准写法；已按生产口径窗口实拉 5 期平安历史账单验证拉取与解析全部正确。顺带将 IMAP 头部拉取改为精确 UID 列表批量 FETCH（每批 50 封，往返从数千次降至数十次；批量失败降级逐封重试不丢信；不再跨批缓存原始头部防内存膨胀）。
- 续费日历信用卡事件按银行聚合：同一银行多张卡还款日相同（同一银行账单日/还款日规则一致）时合并显示为一条「XX银行信用卡」（如「民生银行信用卡」；银行名称别名如「民生」「中国民生银行」归一化到同一银行后合并，标题用规范银行名），点击查看该组第一张卡的详情；单卡仍显示卡名；iCal 订阅保持按卡逐条输出不变。
- 常用货币新增澳门元（MOP，MOP$）：老库升级时自动拉取官方汇率幂等补齐（与玻利维亚诺同一机制），官方报价缺失时推迟到下次启动重试。
- 修复建行账单解析两处勾稽误报 mismatch（生产实测 3~7 月全部误报）：①账户级合并勾稽改为逐卡勾稽——排除「按卡转账还款」分录后的非还款交易净额 == 该卡应还；溢缴款卡（应还为负，滚动余额口径）跳过勾稽不判失败 ②银联扫码通道交易行的卡号列是「6714/扫码」格式，此前未匹配到四位尾号被整行丢弃——现归入对应卡（生产实测漏 1,498.72 扫码消费）③重新解析已有账单时同步重建交易明细（解析器修复后旧记录的缺失明细可补齐，用户还款标记保留）。
- 修复免年费历史账单补拉在邮箱邮件量较大时全部「未找到」：补拉的区间搜索（带 BEFORE 上界）头部扫描上限由「每文件夹最新 200 封」放宽至 2000 封——生产实测建行 2025 年历史账单因 UID 排序进不了 200 封扫描窗口；超过 2000 封安全上限时明确报「扫描不完整，请整理邮箱后重试」，绝不伪装成「未找到」；开放式最近搜索仍保留 200 封上限、行为不变。
- 新增免年费缺失账单补拉：信用卡详情的免年费进度块可一键按缺失期次逐月从绑定的邮箱补拉历史账单（IMAP 按月分段搜索、强制目标银行过滤、历史模式不回写卡片资料防止旧账单覆盖当前资料），每期补拉结果与失败原因（未找到/解析失败/勾稽失败/尾号未匹配）实时展示，重复补拉幂等。
- 新增信用卡免年费进度（可选配置，详情展示）：卡片可登记年费收取日（银行每年收年费的日期，可填未来日期；统计窗口永远是包含今天的最近一个年费周期（收取日在未来时即 [收取日−1年, 收取日)，当前账单计入下一次收费前的周期），缺账单警示只统计账单日已过的期次（当天未出账不计），未到账期的月份不报缺）与「刷 N 笔 / 满 M 元」目标（满足其一即达标，分期计入、退款抵扣金额）；表单校验失败信息修复为可见（此前渲染成空白导致「点了保存没反应」的假象），系统按年费周期自动统计已解析账单达成进度，达标后详情显示「年费可豁免」；检测到年费入账交易时红字提示核实银行是否已豁免，周期内缺账单期时黄色警示进度可能偏低；统计口径以银行实际规则为准。
- 待还款为负数（溢缴款/多还/退款冲抵）时不再显示负数：卡片显示绿色「账上有富余 +金额」，纯富余卡不提供标记按钮（富余留在净额抵扣后续账单）；净额富余但仍有逾期欠款时保留「标记已结清」按钮并红标注明「另欠 x 已逾期 n 天」，确认弹窗披露欠款也在处理范围；统计块仅在无逾期欠款时按富余口径展示（跨卡净额不掩盖他卡欠款）；逾期口径统一为只统计正金额账单（汇总与账单明细一致），富余期次不算逾期。
- 信用卡列表默认按计划还款日由近到远排序（停用卡沉底）；标记已还款顺延到下期后，卡片顺序自动刷新，最需要先还的卡始终排最前。
- 信用卡账单展示按月份命名（「26年8月账单」，取账单出账月，与账单邮件口径一致），待还汇总、确认弹窗、按钮提示同步改为按月份列出，不再使用「n 期」计数。
- 新增账单逾期识别与展示：已出账单未标记还款且还款日已过即为逾期，卡片上显示红色「已逾期 n 天」徽标、待还金额红色强调，账单明细行与待还统计（逾期金额合计、逾期月份）同步展示；标记已还款后逾期状态自动消除并按既有逻辑顺延。
- 信用卡卡片信息分层：卡片只保留名称/银行尾号、周期轨道、待还金额与操作按钮；提前提醒天数、免息期、总额度、名义出账日/还款日移入详情（详情信息格增至六格）。
- 标记已还款的信用卡自动顺延到下个账单周期：卡片上的计划还款日、倒计时、周期轨道、统计排序与站内/订阅日历（iCal）都指向下一期；当期已设定的各提前提醒不再发送，已进入通知队列但未投递的当期提醒在投递前自动取消（通知中心可见「已取消」）。取消单期还款标记只把金额加回待还总额，不回拨周期。新账单期（due_date 晚于已还界线）出现后顺延自然失效。
- 信用卡统计新增「待还款总额」：汇总所有已出账单但未标记还款的每张卡金额（含同卡多期与已删卡的历史账单；勾稽异常的账单不计入），卡片上提供「标记已还款」按钮把该卡全部未还账单一次标记并从总额实时剔除，账单明细区也可单期标记/取消；重新解析同一封账单邮件不会覆盖用户的还款标记，标记随备份导出/恢复。
- 删除信用卡或删除邮件账户不再删除已解析的历史账单与交易明细：改为解除关联并保留（含卡名/尾号/银行标识），新增用户级历史账单接口（/api/credit-cards/statements/all 及明细子接口）支持查询含已删卡在内的全部账单；重新添加同一邮箱并解析时自动认领历史账单（不产生重复记录）。
- 新增账单自动抓取调度：每张信用卡在账单日次日 23:50 自动抓取最新账单邮件并解析（沿用与手动解析相同的解析 / 勾稽 / 落库 / 卡片资料更新链路），银行未发信或解析失败则次一天 23:50 重试，最多连续 3 天，成功即停止后续尝试；停机重启时若仍在重试窗口内补做当天一次、不回放历史。抓取成功与连续 3 次失败各推送一次通知（仅说明卡名与操作结果，不含金额 / 额度 / 尾号 / 账务状态）。此为首个自动轮询任务，此前「不做自动轮询」承诺相应调整。
- 账单解析新增卡片资料自动更新：解析成功的账单以最新邮件为准，自动覆盖对应卡片的账单日 / 还款日 / 总额度（具体日期转为名义日），更新明细在解析结果中展示；勾稽失败的账单不参与更新，卡名 / 银行 / 尾号不受影响；同卡多期账单取最新一期。并修复平安账单「应还金额显示 0」（解析器漏赋值 total_due / min_due）。

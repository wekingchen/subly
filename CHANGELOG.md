...
- 修复民生银行多卡账单逐卡金额错误（生产反馈）：账户级应还合计此前被原样赋给每一张卡——两张不同卡号的卡显示相同的还款金额；现按卡号末四位逐卡派生应还（该卡本期新增净额，退款负净额向正净额卡抵扣保持各卡非负，各卡之和与账单总应还勾稽闭环）、最低还款按净额比例分摊（尾差补齐）；单卡账户无分卡歧义直接用账户级金额；上期未结清（滚动余额）或账户合计解析失败时逐卡金额置空（不把未知渲染成 0、不让派生金额绕过勾稽）。存量数据更新容器后在设置页点「解析账单」（或免年费补拉）即可按新口径重解析刷新。

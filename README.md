...
| 📬 **邮件账户（IMAP）** | 设置页可绑定多个 126 / QQ 邮箱（IMAP 授权码只写入不回显，任何 API 不返回），支持连接测试与手动拉取最近邮件头部预览（发件人 / 主题 / 日期），可按账户配置账单银行白名单（首期招 / 平 / 民 / 中信 / 建 5 家，按发件人域名过滤），为信用卡账单邮件解析做准备；不做自动轮询，邮件预览不落库 |

...
- 新增信用卡免年费进度（可选配置，详情展示）：卡片可登记核卡日与「刷 N 笔 / 满 M 元」目标（满足其一即达标，分期计入、退款抵扣金额），系统按年费周期自动统计已解析账单达成进度，达标后详情显示「年费可豁免」；检测到年费入账交易时红字提示核实银行是否已豁免，周期内缺账单期时黄色警示进度可能偏低；统计口径以银行实际规则为准。

...
test('通知投递中心展示六种状态、尝试历史与受控重发', async ({ page }) => {
  ...
  await page.route('**/api/notifications/outbox/3/attempts', async (route) => {
  ...
  await page.route('**/api/notifications/outbox/3/retry', async (route) => {
    retryCalls += 1
  ...
  await page.route('**/api/notifications/outbox/3', async (route) => {
...
test('重发 409 后刷新行状态，390px 页面不横向溢出', async ({ page }) => {
  ...
  await page.route('**/api/notifications/outbox/5/retry', async (route) => {
    item = { ...item, status: 'sent', sent_at: '2024-01-01T01:03:00Z' }
    await route.fulfill({ status: 409, json: { detail: '当前状态不可重新发送' } })
  })
  await page.route('**/api/notifications/outbox/5', async (route) => {

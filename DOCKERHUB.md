...
- `/app/data` — SQLite database file, uploaded icons, and built-in icon-library cache. **Persist this directory.** The entrypoint repairs ownership of legacy root-owned volume contents, then immediately drops to UID/GID `10001` for Uvicorn. Named volumes and bind mounts are supported when the host filesystem allows ownership changes; deployments overriding Docker `user:` must pre-grant `10001:10001` write access.

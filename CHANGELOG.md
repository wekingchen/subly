...
- 新增邮件账户（IMAP）能力：设置页可绑定 126 / QQ 邮箱，IMAP 授权码只写入不回显（任何 API 均不返回该字段，仅暴露 imap_configured 状态），支持连接测试与手动拉取最近邮件头部预览（发件人 / 主题 / 日期，RFC2047 解码、按 UID 倒序）；拉取仅按时间窗口（1–90 天、最多 50 封），不落库、不自动轮询、不做账单解析，为后续信用卡账单邮件解析打地基；IMAP 凭据不进入任何备份导出。
### Added

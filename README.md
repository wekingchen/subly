...
| 💳 **订阅账本** | 周期订阅 + 一次性买断，支持名称 / 套餐 / 备注搜索、类型与续费风险组合筛选；支持套餐名、个性备注、URL、VPS IPv4 / IPv6、家庭成员、套餐包、日历开关、同分类排序与可选结束日期；分类内默认按剩余订阅时间由近及远排序（无到期日沉底），手动拖拽的顺序跨刷新/跨设备持久化，高级字段通过“更多设置”渐进展开；结束日期当天仍有效，之后停止当前计费、预测与提醒；每次续费自动留痕，详情内可折叠查看完整续费历史；订阅可暂停（不计支出/不提醒/不进日历，账本可见可恢复） |
...
| 🏦 **信用卡还款提醒** | 集中管理多张信用卡的账单日与计划还款日：只登记银行名称、可选尾号、名义账单日 / 还款日与提醒天数，不存储卡号、CVV、有效期或网银凭据；尾号支持一次填多个（如 `1234,2234`）自动拆分创建多张卡；发卡银行匹配招商 / 平安 / 民生 / 中信 / 建设时自动显示官网抓取的官方徽标（本地缓存，失败回退字标）；可选记录总额度（仅展示，不进入提醒与日历）；账单按月份命名（「26年8月账单」）；逾期账单（未标记还款且过还款日）在卡片与明细红色标注「已逾期 n 天」；统计区实时汇总「待还款总额」（每卡以最新一期未标记还款的账单为准——账单应还总额是滚动余额、已含历史欠款；富余卡的负数不计入总额，勾稽异常不计入），卡片上一键标记已还款并从总额剔除，明细区可单期标记 / 取消；页头「全部账单」按钮（或点击待还款统计卡）打开跨卡对账视图——按时间倒序列出全部已解析历史账单（含已删卡的孤立账单，标注「已删卡 / 未关联 · 银行 ···· 尾号」），同样支持单期标记 / 取消与部分还款，标记后待还与逾期统计实时刷新；列表为历史存档、各期金额不可直接相加，最多展示最近 500 条；支持部分还款——还款弹窗输入当次金额（默认填剩余待还全额、焦点清空便于键入），未还清时待还金额与提醒文案同步扣减、账单明细显示「已还 X / 待还 Y」，能可靠确定账单所属还款期时累计还清后自动顺延、并把金额已滚入本期的更早未标记账单一并结清（避免旧期欠款复活）；还款日缺失或历史账单期无法可靠判定时保守不顺延（宁多提醒一期，不错误静默其他未还账单的提醒），取消还款标记会清除该账单已登记的还款记录（清零重算，有确认）；最新账单为负（溢缴款 / 多还）时按「账上有富余」绿色展示而非负数且不抵扣他卡欠款，纯富余卡无需标记操作，逾期统计汇总按最新一期账单、账单明细仍逐期标示；历史补拉的旧账单自动标记已还款（最新一期仍需手动标记）；标记已还款后卡片自动顺延到下个账单周期（还款日 / 倒计时 / 日历 / 提醒全部指向下一期），当期各提前提醒不再发送；卡片列表默认按计划还款日由近到远排序（停用卡沉底），标记还款后顺序自动刷新；卡片只呈现核心信息（周期轨道 + 待还金额），提醒天数 / 免息期 / 额度等细节在详情；可选配置年费收取日（银行每年收年费的日期，可填未来日期）与「刷 N 笔 / 满 M 元」目标（满足其一即达标，分期计入、退款抵扣金额），按年费周期自动统计已解析账单的达成进度，达标显示「年费可豁免」，检测到年费入账或缺账单期时如实提示（以银行实际规则为准）；缺账单期可一键补拉——按缺失期次逐月向绑定的邮箱搜索该银行的账单邮件（历史模式不回写卡片资料），每期补拉结果与失败原因实时可见；系统按月末锚定规则自动推导每期「账单日 → 计划还款日」周期（31 日在短月自动取月末），账单日至还款日的自然日间隔仅作展示、不等同于银行免息期；计划还款事件与订阅续费共享日历与 iCal，提醒走同一套 Telegram / Bark / Webhook 可靠投递，文案含银行与卡名、倒计时及最新未还账单的应还金额（金额未知时提示以银行账单为准；交易明细、额度与尾号仍不外发） |
...
| 🔔 **Bark 推送** | iOS 推送提醒，与 Telegram 可同时开启、各发各的；支持订阅图片图标（无图标回退 Subly 品牌 logo）、信用卡银行徽标、自建服务器、提示音、分组与可选 TTL |
...
| 💱 **多货币** | 全球主流货币（含澳门元 MOP 等 23 种常用货币）+ 自定义货币；设置页可维护自定义名称、符号及「1 自定义币 = X 用户基准币」手动汇率，汇率缓存与每日自动刷新，按用户基准货币统计；自定义货币需先设置可用汇率才能用于订阅或基准币，使用中不能清空汇率 |
...
| `APP_PUBLIC_URL` | | 对外可访问地址（如 `https://subly.example.com`）；用于生成公网日历服务可订阅的私有 iCal URL、Bark 测试推送跳转，以及把上传/内置订阅图标、品牌 logo、信用卡银行徽标转换为 Bark 设备可下载的绝对 URL（地址必须能被接收推送的设备访问）；真实提醒点击地址仍取订阅 `url` |
...
Bark 推送的 Device Key、服务器、提示音、分组与 TTL 均在网页「设置」里按用户配置。iOS 15+ 可显示推送图片图标：订阅自有图标优先，无图标时回退 Subly 品牌 logo；信用卡还款提醒使用所属银行官方徽标（未收录银行同样回退品牌 logo）。上传/内置图标与 logo 需要配置 `APP_PUBLIC_URL`，且该地址必须能被接收 Bark 的设备访问。未配置或图标不可用时提醒仍会正常送达，只显示 Bark 默认图标；Emoji/普通文本不会作为 Bark 图标发送。真实提醒点击地址仍取订阅自身 `url`。完整示例见 [.env.example](./.env.example)。
